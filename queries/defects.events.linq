<Query Kind="Program">
  ...
  <Namespace>Dgg.Cqrs.Sample.Core.Domain.DefectHandling.Events</Namespace>
</Query>
...
{
	using(var db = Db4oEmbedded.OpenFile(eventsConnection)){
		foreach(var e in db.Cast<Dgg.Cqrs.Sample.Core.Infrastructure.Eventing.DomainEvent>().Where(e => e.GetType().Name.Contains("Issue"))) db.Delete(e);
		db.Commit();
	}
}
